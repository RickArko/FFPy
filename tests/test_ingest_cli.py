...
class TestCliArgparse:
    ...
    def test_json_flag_after_subcommand(self):
        args = self.parser.parse_args(["espn", "123456", "--json"])
        assert args.format == "json"

    def test_csv_flag_after_subcommand(self):
        args = self.parser.parse_args(["yahoo", "389.l.12345", "--csv"])
        assert args.format == "csv"

    def test_db_flag_with_path_after_subcommand(self):
        args = self.parser.parse_args(["sleeper", "league1", "--db", "/tmp/test.db"])
        assert args.db == "/tmp/test.db"

    def test_db_flag_alone_means_default_path(self):
        args = self.parser.parse_args(["sleeper", "league1", "--db"])
        assert args.db == ""
    ...
    def test_matchups(self):
        ...
        assert args.league_id == "espn:123456"
        assert args.week == 3
        matchups_parser = self.parser.parse_args(["matchups", "espn:1", "1"])
        assert matchups_parser.week == 1
